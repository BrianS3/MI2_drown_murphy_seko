...
data = pd.read_csv('assets/MI1_TWEETS.csv')
...
def preprocess_tweets(csv):
    '''
    INPUT: tweet csv
    cleans tweets;
    detects tweet language and translates to English;
    isolates hashtags;
    lemmatizes tweets
    OUTPUT: original pandas dataframe with added columns TIDY_TWEET, TWEET_LANG, HASHTAGS, LEMM
    '''
...
#         data = data[data.TWEET_LANG == 'en'] # removing non-English tweets
    ...
    # get fully preprocessed df
    new_df = lemmatize(clean)
...
clean = preprocess_tweets('assets/MI1_TWEETS.csv')
...
model.build_vocab(clean['LEMM'])


# In[48]:


model.train(clean['LEMM'], total_examples=model.corpus_count, epochs=500)


# In[49]:


model.wv.most_similar(positive=['ebola'])
...
cluster_model = KMeans(n_clusters=3, random_state=42).fit(X=word_vectors.vectors)


# In[42]:


word_vectors.similar_by_vector(cluster_model.cluster_centers_[1], topn=50)
